...
if [ -n "${CONFIG}" ]; then
	...
	cat ${AUTOSCALER_VMWARE}/config/${CONFIG}/config/kubernetes-cloud-autoscaler.json | jq \
		...
		'. | .listen = "/var/run/cluster-autoscaler/vmware.sock" | .network = "unix" | ."src-etcd-ssl-dir" = $ETCD_SSL_DIR | ."kubernetes-pki-srcdir" = $PKI_DIR | ."ssh-infos"."ssh-private-key" = $SSH_KEY' > ${CONFIG_DIR}/kubernetes-cloud-autoscaler.json
fi
