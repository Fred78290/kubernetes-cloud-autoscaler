	...
	if conf.providerConfiguration == nil {
		if conf.Plateform == nil {
			...
			case providers.LxdProviderName:
				conf.providerConfiguration, err = lxd.NewLxdProviderConfiguration(configFile)
			default:
...
func (cfg *Config) ParseFlags(args []string, version string) error {
	...
	app.Flag("max-nodes", "Limits: max nodes (default: 24)").Default(strconv.FormatInt(cfg.MaxNode, 10)).Int64Var(&cfg.MaxNode)
